...
GENERATE_HWY_FILES_INPUTS_DIR_PATH = os.path.join(
    INPUTS_DIR_PATH, "generate_transit_files"
)
...
GENERATE_TRANSIT_FILES_PY_PATH = os.path.join(
    TRAVEL_SCRIPTS_DIR_PATH, "4_generate_transit_files.py"
)
...
def _check_can_run_tool():
    """
    Helper function: `generate_transit_files` tool requires
    that `export_future_hwys` tool has been run (I believe).
    Simple wrapper function around `_check_export_future_hwys_run` function
    to make sure that the tests for `generate_transit_files`
    can be run properly.
    """
...
def test_run_full_script_no_flags():
    ...
    """
    command = [ARCPY_PYTHON_PATH, GENERATE_TRANSIT_FILES_PY_PATH]
